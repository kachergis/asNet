...
vocab_data <- instruments_inc %>%
  mutate(inst_vocab_data = purrr::map2(language, form, get_vocab_data))

vocab_comp_category <- vocab_data %>%
  mutate(vocab_comp = purrr::map(inst_vocab_data, ~get_vocab_comp(., category)))
...
demo_predictions_TD <- vocab_comp_data_TD %>%
  group_by(category) %>%
  nest() %>%
  mutate(model = purrr::map(data, ~clm(prop ~ I(vocab ^ 3) + I(vocab ^ 2) + vocab - 1, data = .)),
         predictions = purrr::map(model, ~broom::augment(., newdata = tibble(vocab = pts)))) %>%
...
#feather::write_feather(demo_predictions_all_c, "saved_data/demo_predictions_all_c_WS")
...
items_as <- read_feather("saved_data/all_aoa_WG.feather") %>%
  select(-item)
...
demo_predictions_TD <- vocab_comp_data_TD %>%
  group_by(category) %>%
  nest() %>%
  mutate(model = purrr::purrr::map(data, ~clm(prop ~ I(vocab ^ 3) + I(vocab ^ 2) + vocab - 1, data = .)),
         predictions = purrr::purrr::map(model, ~broom::augment(., newdata = tibble(vocab = pts)))) %>%
  dplyr::select(c(category, predictions))%>%
  unnest() %>%
  rename(prop = .fitted)%>%
  mutate(level = "TD")
...
demo_predictions_AS <- vocab_comp_data_AS %>%
  ...
  mutate(model = purrr::map(data, ~clm(prop ~ I(vocab ^ 3) + I(vocab ^ 2) + vocab - 1, data = .)),
         predictions = purrr::map(model, ~broom::augment(., newdata = tibble(vocab = pts)))) %>%
  dplyr::select(category, predictions) %>%
  unnest() %>%
  rename(prop = .fitted)%>%
  mutate(level = "AS")
...
# feather::write_feather(demo_predictions_all_c, "saved_data/demo_predictions_all_c_WG")
