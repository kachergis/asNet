...
```{r}
d_demo %>% ggplot(aes(x=age, y=production, group=group, color=group)) +
...
```{r load-ws-model}
load("data/prodWS_IRT_model.Rds")
...
big_dif %>% arrange(d_diff) %>% 
  ...
  kable(format = "html", table.attr = "style='width:30%;'", digits=2)
```
...
sort(setdiff(wg_items$definition, asd_items$definition)) # back throw <- two WG items
sort(setdiff(asd_items$definition, wg_items$definition)) # backyard_1  <- 1 unmatched NDAR item..
