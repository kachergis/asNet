...
GK ToDo: still need to add collection_id, dataset_id, test_dat, sex, and src_subject_id to WG ASD data

```{r clean-asd-wg, eval=F}
data_AS_WG <- read.delim("data/ASD CDI data/mci_words_gestures01.txt", header = TRUE, sep = "\t", dec = ".") %>% filter(mcg_vc_totcom != "999") # or _totpr for production?
...
p2 <- ggplot(data=demo_asd_wg, aes(x=jitter(age))) +
  geom_point(data=demo_asd_wg, alpha=.1, aes(y=production)) + 
